/**
 * Markdown text-editor TUI component used by chain-clarify and the
 * agent edit screens. Pure imperative — replaces text-editor.ts.
 */
...
export function handleEditorInput(
	...
	if (matchesKey(data, "up")) {
		if (cursorPos.line > 0) {
			...
			return { ...state, cursor: (starts[targetLine] ?? 0) + targetCol };
		}
		return state;
	}
	...
	if (matchesKey(data, "home")) return { ...state, cursor: starts[cursorPos.line] ?? 0 };
	if (matchesKey(data, "end")) return { ...state, cursor: (starts[cursorPos.line] ?? 0) + (wrapped[cursorPos.line]?.length ?? 0) };
